...
export function PostDetail({ post }: { post: BlogPost }) {
  ...
  return (
    <>
      <section className="bg-vv-bg-warm border-b border-vv-line" data-screen-label="01 Post Hero">
        <Container className="max-w-220">
          <div className="font-code text-vv-muted text-[12px] tracking-[0.06em] mb-6">
            ...
            <Link href="/blog" className="hover:text-vv-ink">Blog</Link>{" "}
          ...
          <div className="mt-4 flex items-center gap-3">
            ...
            <span className="text-[12px] text-vv-ink-2">{post.readingTime}</span>
      ...
      <section className="border-t border-vv-line py-12" data-screen-label="02 Post Body">
        <Container className="max-w-220">
      ...
      {/* Related posts */}
      {related.length > 0 && (
        <section className="border-t border-vv-line bg-vv-bg-warm py-12" data-screen-label="03 Related">
          <Container className="max-w-220">
            ...
            <div className="grid gap-5 sm:grid-cols-2">
              {related.map((p) => (
                <article key={p.slug} className="group flex flex-col overflow-hidden rounded-[22px] border border-vv-line bg-vv-bg transition hover:border-vv-accent">
                  ...
                  <div className="flex flex-col gap-2 p-5 flex-1">
                    <span className="text-[11px] font-semibold text-vv-accent-deep">{p.category}</span>
                    <h3 className="text-[16px] font-semibold text-vv-ink flex-1">{p.title}</h3>
                    <Link href={`/blog/${p.slug}`} className="text-[13px] font-medium text-vv-ink hover:text-vv-accent-deep">
                      Read more →
                    </Link>
                  </div>
                </article>
            ...
            <div className="mt-6">
              <Link href="/blog" className="text-[13px] text-vv-ink-2 underline hover:text-vv-ink">
                ← All posts
              </Link>
            </div>
          </Container>
